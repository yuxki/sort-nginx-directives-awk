...
result=`$awk -f sort-nginx-directives.awk tests/input.conf | sort -t: -k 1,1n`
...
result_include=`$awk -f sort-nginx-directives.awk -v "find_path_opt_include=on" tests/input-include.conf | sort -t: -k 1,1n`

if [ "$expect_include" != "$result_include" ]; then
  echo "[ERROR]: Unexpected output is found with find_path_opt_include=on option."
  exit 1
fi
